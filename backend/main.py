...
async def predict_disease(file: UploadFile = File(...)):
    ...
    try:
        ...
        if crop_disease_model is not None:
            ...
            # Map the index to disease name - this is a placeholder
            # In a real implementation, you would have a mapping from class indices to disease names
            diseases = list(disease_info.keys())
            if predicted_class_index < len(diseases):
                disease_name = diseases[predicted_class_index]
            else:
                disease_name = "Unknown Disease"
        else:
            # If model is not loaded, return a sample prediction
            disease_name = "Bacterial Leaf Blight"  # Sample disease for testing
            confidence = 85.5  # Sample confidence for testing
        
        # Get disease information
        disease_data = disease_info.get(disease_name, {
            "description": "Unknown disease",
            "recommendations": "Consult a plant pathology expert",
